...
object Generators {
    ...
    /**
     * Randomly create a date between the given start and end dates. For better performance, we convert this
     * to a long instead of a date, string, etc.
     */
    fun generateDateCreated(dateStart: LocalDate, dateEnd: LocalDate): Date {
        val starting = dateStart.toLong()
        val ending = dateEnd.toLong()
     ...
     */
    fun generateLog(dateCreated: Date, mainFailMessage: String): List<String> {
        val messages = listOf(
        ...
        val messageType = listOf(" INFO", " WARN", "DEBUG")
        val dateFormat = SimpleDateFormat("yyyy-MM-dd hh:mm:ss")

        //to convert Date to String, use format method of SimpleDateFormat class.
        val strDate = dateFormat.format(dateCreated)
        return (0..20).map { "$strDate  ${messageType.random()} --- ${messages.random()}" } + "$strDate  ERROR --- $mainFailMessage"
    }

    fun generateElasticSearchEntry(): ElasticSearchEntry {
        val formatter = DateTimeFormatter.ofPattern("yyyy-MM-dd")
        val startDate = stringToLocalDate("2019-02-01")
        val endDate = stringToLocalDate("2019-02-28")
        val dateCreated = generateDateCreated(startDate, endDate)
        val mainFailMessage = generateMainFailMessage()
        return ElasticSearchEntry(
                ...
                mainFailMessage = mainFailMessage,
                sha = shas.random(),
                sources = generateSources(),
                log = generateLog(dateCreated, mainFailMessage)
        )
    }
}